document.addEventListener('DOMContentLoaded', function () {
    ...
    function generateMatrixInputs(rows, cols) {
        ...
        // Add horizontal label (Assets Provisions)
        const headerRow = document.createElement('tr');
        headerRow.innerHTML = `<th></th><th colspan="${cols}">Assets Provisions</th>`;
        thead.appendChild(headerRow);

        // Add column headers
        const labelRow = document.createElement('tr');
        labelRow.innerHTML = '<th>Best Practices</th>' + Array.from({ length: cols }, (_, i) => `<th>${i + 1}</th>`).join('');
        thead.appendChild(labelRow);

        // Add input fields for matrix
        for (let i = 0; i < rows; i++) {
            const row = document.createElement('tr');
            row.innerHTML = `<th>${i + 1}</th>` + Array.from({ length: cols }, (_, j) =>
                `<td><input type="number" id="r${i}c${j}" step="0.0001" value="0" onchange="updateSums(${i}, ${j})" oninput="validateInput(this)"></td>`
            ).join('');
